...
{
    // בדיקת היום הנוכחי
    int todayNumber = DayOfWeek();
    string currentDayName = GetDayName(todayNumber);
    ...
    for(int i = 0; i < symbolsTotal; i++)
    {
        ...
        bool isTripleSwapToday = (todayNumber == tripleSwapDay);
        ...
        if(spreadCostUSD < (actualSwapToday / 2.0) && actualSwapToday > 0)
        {
            ...
            {
                profitablePairs += "   *** היום ריבית משולשת! ***\n";
...
int GetTripleSwapDay(string symbol)
{
    ...
    long longValue = SymbolInfoInteger(symbol, 40); // 40 = SYMBOL_SWAP_ROLLOVER3DAYS
    int tripleSwapDay = (int)longValue;
    ...
    {
        return 3; // רביעי לצמדי מט"ח ומתכות
    }
    else
    {
